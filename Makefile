...
flow-storm-dbg.jar:
	clj -X:build hf.depstar/jar :jar flow-storm-dbg.jar :aliases '[:dbg]' :paths-only false :sync-pom true :version '"$(VERSION)"' :group-id jpmonettas :artifact-id flow-storm-dbg;

flow-storm-inst.jar:
	clj -X:build hf.depstar/jar :jar flow-storm-inst.jar :aliases '[:inst]' :sync-pom true :version '"$(VERSION)"' :group-id jpmonettas :artifact-id flow-storm-inst;
