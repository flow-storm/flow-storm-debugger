...
when you already have something to trace. If this is the case you can use
(flow-storm.api/remote-connect {:on-connected (fn [] (my-app-start))}) so you start your application after
...
.Reconnecting
====
When you are remote debugging (Clojure or ClojureScript) and the debuggee restarts (like a browser page reload)
you can reconnect to it without restarting the debugger by clicking at the reconnect button on the toolbar.
====
...
- Reconnect to repl server (useful for page reloads in ClojureScript)
...
- `Ctrl-k` Clean all debugger state
