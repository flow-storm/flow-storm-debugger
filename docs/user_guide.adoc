...
If your application is running in a different box you can also provide two extra keys :
   ...
   - `:debugger-host` should be the ip of the debuggee (defaults to localhost)
   - `:runtime-host` shoud be the ip where the debugger is running, since the debuggee needs to connect back to it (defaults to localhost)"
