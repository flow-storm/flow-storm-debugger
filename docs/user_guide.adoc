...
The simplest way is to setup it globally so that is what we are going to do next, but you can also add it only
...
Then you can start your repls with `:1.12-storm` alias (like `clj -A::1.12-storm`) and when the repl comes up evaluate the `:dbg` keyword to bring up the UI,
...
Then you can start your repls with `+1.12-storm` profile (like `lein with-profile +1.12-storm repl`) and when the repl comes up evaluate the `:dbg` keyword to bring up the UI,
then click on  `Help->Tutorial` on the menu for a tour of the basics.
