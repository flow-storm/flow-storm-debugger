...
If you are using ClojureStorm you can jump to the last captured exception by evaluating the `:ex` keyword on your repl. This
will position the debugger right before the exception so you can step backwards and explore the cause.

If you are using vanilla or using FlowStorm with ClojureScript, most of the time you can accomplish the same by clicking the `Step last` button
