...
This is the newest and simplest way of using FlowStorm, but requires you to swap your oficial Clojure compiler by ClojureStorm.
Swapping compilers sounds like a lot but don't worry, ClojureStorm is just a patch applied over the oficial compiler with some
...
{:paths ["src"]
 :deps {}
 :aliases {:dev {:classpath-overrides {org.clojure/clojure nil} ;; for disabling the oficial compiler
...
This is probably what most people want since you don't want to instrument things like nrepl, cider or any of your tooling stuffm
...
(defproject my.project "1.0.0"
  :profiles {:dev {:dependencies [[com.github.jpmonettas/clojure "RELEASE"]
                   ...
                   :exclusions [org.clojure/clojure] ;; for disabling the oficial compiler
                   :jvm-opts ["-Dclojure.storm.traceEnable=true"
                              "-Dclojure.storm.instrumentEnable=true"
                              "-Dclojure.storm.instrumentOnlyPrefixes=YOUR_INSTRUMENTATION_STRING"]}}
  :main foo.core)
...
`flow-storm-inst` instead of `flow-storm-dbg` being the former a slimed down version of the later one that
doesn't contain some libraries used only by the UI, but using the full `flow-storm-dbg` is also ok.

Once you have on your remote box your debugge running and exposing a nrepl server you can connect to it by running :
