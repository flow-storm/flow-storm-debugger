...
----
;; on Linux and OSX
clj -Sforce -Sdeps '{:deps {} :aliases {:dev {:classpath-overrides {org.clojure/clojure nil} :extra-deps {com.github.flow-storm/clojure {:mvn/version "1.12.0-5"} com.github.flow-storm/flow-storm-dbg {:mvn/version "4.2.0"}} :jvm-opts ["-Dclojure.storm.instrumentEnable=true"]}}}' -A:dev
...
After the tutorial you would want to add and configure it for your projects, or <<#_a_global_configuration, create a global config>> which the
following sections describe.

==== Setup with deps.edn

If your project is using deps.edn, you should modify it to look like this :
...
{:paths ["src"]
 ...
 :aliases {:dev {;; for disabling the official compiler
                 ...
                 :extra-deps {com.github.flow-storm/clojure {:mvn/version "1.12.0-5"}
...
Once you have setup your deps.edn, start your repl with the `:dev` alias and run the debugger by evaluating
...
(defproject my.project "1.0.0"
  :profiles {:dev {:dependencies [[com.github.flow-storm/clojure "1.12.0-5"]
...
Once you have setup your project.clj, start your repl with the `dev` profile and run the debugger by evaluating
the `:dbg` keyworkd on your repl (this means just type `:dbg` and hit return).

By default lein adds the dev profile but if you name it something else, like `flowstorm`, make sure you activate the profile
with `lein with-profile +flowstorm repl`. 
...
----
{:flowstorm
 {:dependencies [[com.github.flow-storm/clojure "1.12.0-5"]
                 [com.github.flow-storm/flow-storm-dbg "4.2.0"]]
  :exclusions [org.clojure/clojure]
  :jvm-opts ["-Dflowstorm.startRecording=false"
             "-Dclojure.storm.instrumentEnable=true"
...
Then you can you can activate the profile with `lein with-profile +flowstorm repl`.
...
FlowStorm works ok with most macros, but in order to link what gets executed to your source code it adds meta data on the forms
before macroexpansion so after it can link the macro expanded forms to the source code.

Macros can be as simple as code reorganizing ones (like ->, ->>, when, and, defn, etc) or whole compilers like clojure.core.async/go and electric.

For the later, if the macro doesn't preserve the meta from the original sub-forms, there is no way to link executed code to the original expressions,
so the code will run as normal but you will not get all the expressions captured.

This sometimes can be fixed on the macros themselves, making sure it preserves all meta after macro expansion.
