...
.ClojureStorm
====
Currently, working with different flows in parallel is only supported in Vanilla FlowStorm.
If you are using ClojureStorm or ClojureScriptStorm the only flow you will see is the funnel flow.
====
...
- The <<#_searching,value search>> tool
...
You can use the search box under the controls to search for the next expression (starting at your current step) which its string
serialization contains your search text.

You can use the print-level and print-length fields to control how deep this serialization will be made. If these numbers are big
...
see where most of your traces are coming from. If you want to reduce the number of traces, to make let's say, search faster,
...
recorded expression. It will ask you for an optional message, which you can use to identify these particular prints in the output console.
...
Let's say you added/removed a prefix and you want to fire a shadow-cljs recompile of your files so the changes take effect.
...
If you define those, clicking on your forms namespaces link should run the provided commands.
...
If something doesn't work try the command on your shell first, maybe you need the full path to the binary.
...
If the files are inside a jar you can put any command here using the <<JAR>>, <<FILE>>, and <<LINE>> placeholders that
will be executed by FlowStorm to open it. I don't know any for VSCode, if you know how to do it please submit a PR.

[,clojure]
----
"-Dflowstorm.jarEditorCommand="
----
