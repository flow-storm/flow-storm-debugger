...
When you first open FlowStorm UI you will see three things, from left to right :
...
On each flow you can use the search tool to search over all your flow recorded values. Find the search tool under `More tools -> Search`.

image::user_guide_images/search.png[]

The first row let us constrain our search to a specific thread and also let us select one of two
types of searches.
...
Double clicking on any of the search results will take you to the code stepper at that point in time.
...
Double clicking any row will take you to the code at that point in time.
